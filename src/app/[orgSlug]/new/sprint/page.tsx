    ...
    <Modal title={t("newSprint")} subtitle={`L${level} · ${t('recommendedDuration', { level: String(level) })} ${config.label}`} sidebar={SIDEBAR}>
      <form onSubmit={save}>
        ...
        <Field label={t("strategicFocus")} hint={t("focusHint")}>
          <textarea className="input" rows={3} value={form.focus} onChange={set("focus")} onBlur={e => coach.check("focus", e.target.value, org?.id)}
        ...
        <Field label={t("successSignals")} hint={t("signalsHint")}>
          <textarea className="input" rows={2} value={form.signals} onChange={set("signals")} onBlur={e => coach.check("signals", e.target.value, org?.id)}
